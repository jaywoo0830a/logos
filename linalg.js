...
export class Matrix {
  ...
  /** 정사각 행렬인가 */
  get square() { return this.n === this.m; }
  /** 행렬식 (2×2, 3×3) */
  get det() {
    ...
    if (this.n === 3) {
      const [a, b, c] = this.rows[0], [d, e, f] = this.rows[1], [g, h, i] = this.rows[2];
  ...
  /** 열 벡터 j */
  col(j) { return this.rows.map((r) => r[j]); }
  /** 전치 */
  t() { return mat(Array.from({ length: this.m }, (_, j) => this.col(j))); }
  /** 행렬곱 `this · N` */
  mul(N) {
  ...
  /** 점(벡터) 하나에 적용 — `A·x` */
  apply(p) { return this.rows.map((row) => row.reduce((s, v, k) => s + v * p[k], 0)); }
  /** 점 배열에 적용 */
  map(pts) { return pts.map((p) => this.apply(p)); }
  /** 사람이 읽는 형태 */
  toString() { return `mat([${this.rows.map((r) => `[${r.join(', ')}]`).join(', ')}])`; }
}

/** 행렬 생성 — `mat([[a,b],[c,d]])` */
export function mat(rows) { return new Matrix(rows); }

/** 단위행렬 */
mat.identity = (n = 2) => new Matrix(Array.from({ length: n }, (_, i) => Array.from({ length: n }, (_, j) => (i === j ? 1 : 0))));
/** 회전행렬 R(θ) — θ 는 도(반시계) */
mat.rotation = (deg) => {
...
mat.shear = (kx = 0, ky = 0) => mat([[1, kx], [ky, 1]]);
/** 스케일 */
mat.scaling = (sx, sy = sx) => mat([[sx, 0], [0, sy]]);
/** 반사 — 'x'(x축) | 'y'(y축) | 'yx'(y=x) | 도(단위벡터 방향) */
mat.reflection = (axis) => {
  if (axis === 'x') return mat([[1, 0], [0, -1]]);
  if (axis === 'y') return mat([[-1, 0], [0, 1]]);
  if (axis === 'yx') return mat([[0, 1], [1, 0]]);
  const t = (Number(axis) * Math.PI) / 180, c = Math.cos(2 * t), s = Math.sin(2 * t);
  return mat([[c, s], [s, -c]]);
};
...
export const vec = {
  ...
  /** a 와 b 사이의 각(도) */
  angleDeg: (a, b) => (Math.acos(Math.max(-1, Math.min(1, vec.dot(a, b) / (vec.norm(a) * vec.norm(b))))) * 180) / Math.PI,
  /** 3D 외적 (2D 입력은 z=0 으로 승격) */
  cross: (a, b) => {
    const [ax, ay, az = 0] = a, [bx, by, bz = 0] = b;
