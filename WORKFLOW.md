...
# ② 스케치 폴더 만들기 (뼈대 + 예제 1장)
node bin/logos.mjs new sketches

# ③ 렌더 — 기본은 도커에서 실행, 산출물만 호스트로 나온다
...
```
                      호스트(리눅스)                 컨테이너 logos-render:0.1.0
...
SVG 는 정상 생성되고 경고만 남습니다. 이미지에는 포함되어 있으니 `--docker` 로 돌리면 해결됩니다.

**Q. 산출물이 root 소유다** — compose 를 쓸 때 `user:` 를 지정하지 않았을 수 있습니다.
`UID=$(id -u) GID=$(id -g) docker compose run …` 또는 `--docker` 기본 경로를 쓰세요.
...
* 수동 확인(리눅스) — `scripts/render.sh --local` 7 figure · `scripts/render.sh`(도커) 7 figure,
