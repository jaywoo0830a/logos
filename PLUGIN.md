...
```js
class TipMarked extends Drawable {              // 예: plugins/geometry-extras.js
  toIR(ctx) {
    const irs = this.inner.toIR(ctx);
    return irs.map((n, i) => (i === irs.length - 1 && n.kind === 'path'
      ? { ...n, data: { ...n.data, head: true } } : n));
  }
}
api.extend('drawable', { arrowTip() { return new TipMarked(this); } });
...
scene().equal().theme('chalk')
  ...
  .compile().toSVG();          // SVG 와 toTikZ() 둘 다 동작
```
...
**경계**: 기존 렌더링 *알고리즘*(숨은선, 곡선 샘플링, 레이아웃)을 바꾸는 일은 코어의 일입니다.
