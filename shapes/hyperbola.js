...
function branchPath(c, cx, cy, a, b, h, n, signBran, ySign) {
  ...
  for (let i = 0; i <= n; i++) {
    ...
    const px = cx + signBran * x, py = cy + y;
    if (!Number.isFinite(px) || !Number.isFinite(py)) { started = false; continue; }
    ops.push({ op: started ? 'L' : 'M', x: px, y: py });
    started = true;
  }
  return node('path', { ops, color: c.color, stroke: c.stroke, dash: c.dash, opacity: c.opacity, style: pickStyleH(c) });
}

export class Hyperbola extends Drawable {
  constructor(conf = {}) { super('hyperbola', { ...conf }); }
...
export const hyperbola = {
  center(O) { return { semi: (a, b) => new Hyperbola({ center: O, semi: [a, b] }) }; },
};
