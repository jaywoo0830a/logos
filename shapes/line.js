...
export class Line extends Drawable {
  ...
  pointDir() {
    ...
    switch (c.form) {
      ...
      case 'slope': return { p: [0, c.intercept], d: [1, c.m] };
      case 'horizontal': return { p: [0, c.y], d: [1, 0] };
      case 'vertical': return { p: [c.x, 0], d: [0, 1] };
      default: return { p: [0, 0], d: [1, c.m || 0] };
    }
  }
  ...
  clipped(world) {
    ...
    return clipSeg([p[0] - ux * span, p[1] - uy * span],
                   [p[0] + ux * span, p[1] + uy * span], world);
  }
...
function clipSeg([x0, y0], [x1, y1], w) {
  ...
  let x0s = x0, y0s = y0, x1s = x1, y1s = y1;
  let c0 = code(x0s, y0s), c1 = code(x1s, y1s);
  let guard = 0;
  while (true) {
    if (!(c0 | c1)) return [[x0s, y0s], [x1s, y1s]];
    if (c0 & c1) return null;
    const c = c0 ? c0 : c1;
    let x, y;
    if (c & TOP) { x = x0s + (x1s - x0s) * (w.ymax - y0s) / (y1s - y0s); y = w.ymax; }
    else if (c & BOTTOM) { x = x0s + (x1s - x0s) * (w.ymin - y0s) / (y1s - y0s); y = w.ymin; }
    else if (c & RIGHT) { y = y0s + (y1s - y0s) * (w.xmax - x0s) / (x1s - x0s); x = w.xmax; }
    else { y = y0s + (y1s - y0s) * (w.xmin - x0s) / (x1s - x0s); x = w.xmin; }
    if (c === c0) { x0s = x; y0s = y; c0 = code(x0s, y0s); }
    else { x1s = x; y1s = y; c1 = code(x1s, y1s); }
...
export const line = {
  ...
  horizontal(y = 0) { return new Line({ form: 'horizontal', y }); },
  vertical(x = 0) { return new Line({ form: 'vertical', x }); },
  intercepts(xi, yi) { return new Line({ form: 'two-point', a: _point(xi, 0), b: _point(0, yi) }); },
  standard(a, b, c) {
    if (!b || Math.abs(b) < 1e-12) return new Line({ form: 'vertical', x: -c / a });
    return new Line({ form: 'slope', m: -a / b, intercept: -c / b });
  },
  perpendicular(l) { return new Builder(l).perpendicular(); },
  parallel(l) { return new Builder(l).parallel(); },
  tangent(circle) { return new TangentBuilder(circle); },
  angleBisector(A, B, C) {
