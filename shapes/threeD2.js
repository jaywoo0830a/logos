  ...
  toIR(ctx) {
    ...
    const ring = (oz) => {
      ...
      for (let i = 0; i <= 48; i++) {
        ...
        p.push(project3(ctx, [cx + r * Math.cos(a), cy + r * Math.sin(a), oz]));
      }
      return p;
    };
    ...
    for (const a of [0, Math.PI])
      out.push(
        polyline(
          [
            project3(ctx, [cx + r * Math.cos(a), cy + r * Math.sin(a), cz]),
            project3(ctx, [cx + r * Math.cos(a), cy + r * Math.sin(a), cz + h]),
          ],
          this._conf,
          0,
        ),
...
export class Cone extends Drawable {
  ...
  get vertices() {
    ...
    return [
      [vx, vy, vz],
      [vx + dir[0] * h - r, vy + dir[1] * h - r, vz + dir[2] * h - r],
      [vx + dir[0] * h + r, vy + dir[1] * h + r, vz + dir[2] * h + r],
    ].map((v) => point(...v));
  }
  toIR(ctx) {
    ...
    for (let i = 0; i <= 48; i++) {
      const a = (2 * Math.PI * i) / 48;
      ring.push(project3(ctx, [bx + r * Math.cos(a), by + r * Math.sin(a), bz]));
    }
    const out = [polyline(ring, this._conf, -1)];
    const ap = project3(ctx, [vx, vy, vz]);
    for (const a of [0, Math.PI])
      out.push(polyline([ap, project3(ctx, [bx + r * Math.cos(a), by + r * Math.sin(a), bz])], this._conf, 0));
    return out;
  }
}
export const cone = {
  vertex(V) {
    return {
      axis: (a) => ({ radius: (r) => ({ height: (h) => new Cone({ vertex: V, axis: a, radius: r, height: h }) }) }),
      radius: (r) => ({ height: (h) => new Cone({ vertex: V, radius: r, height: h }) }),
    };
  },
};
  ...
  toIR(ctx) {
    ...
    for (let i = 0; i <= n; i++) {
      ...
      for (let j = 0; j <= 32; j++) {
        ...
        ring.push(project3(ctx, [x, y * Math.cos(th), y * Math.sin(th)]));
      }
    ...
    for (const s of [1, -1]) {
      ...
      for (let i = 0; i <= n; i++) {
        ...
        const y = curve.eval(x).cart[1];
        if (!Number.isFinite(y)) continue;
        prof.push(project3(ctx, [x, y * s, 0]));
      }
...
export class ZSurface extends Drawable {
  constructor(conf = {}) {
    super('surface', { xr: [-2, 2], yr: [-2, 2], n: 24, ...conf });
  }
  on(xr, yr) {
    return this.set({ xr, yr });
  }
  mesh(n) {
    return this.set({ n });
  }
  faces(on = true) {
    return this.set({ faces: on });
  }
  /** 면 색을 높이 z 로 컬러맵 적용 (matplotlib cmap 대응) */
  cmap(name) {
    return this.set({ cmap: name });
  }
  ...
  toIR(ctx) {
    ...
    if (c.faces) {
      ...
      for (const q of quads) {
        ...
        out.push(
          node('polygon', {
            pts: q.p.map((p) => [p[0], p[1]]),
            depths: q.p.map((p) => p[2]),
            closed: true,
            fill: col,
            color: col,
            stroke: 0.4,
            opacity: c.opacity ?? 0.96,
            z: depthZ(q.depth),
      ...
      for (let j = 0; j <= n; j++) {
        ...
        const l = line3(pts, c.stroke ?? 1, c.color || base, true);
        if (l) out.push(l);
      }
      for (let i = 0; i <= n; i++) {
        const pts = [];
        for (let j = 0; j <= n; j++) pts.push(P(i, j));
        const l = line3(pts, c.stroke ?? 1, c.color || base, true);
        if (l) out.push(l);
      }
    }
    ...
    for (const b of boundary) {
      const l = line3(b, 1.4, shadeHex(base, 0.7), false);
      if (l) out.push(l);
    }
    return out;
  }
...
export const surface = {
  revolution(curveObj) {
    return { about: () => new Surface({ curve: curveObj }) };
  },
  /** z = f(x,y).on([x0,x1],[y0,y1]).mesh(n) / .faces() */
  z(fn) {
    return new ZSurface({ fn });
  },
};
...
export class VectorField3 extends Drawable {
  constructor(conf = {}) {
    super('vectorField3', { box: [-2, 2, -2, 2, -2, 2], step: 1, len: 0.7, ...conf });
  }
  on(box) {
    return this.set({ box });
  }
      ...
      for (let y = Math.ceil(ay / st) * st; y <= by + 1e-9; y += st) {
        for (let z = Math.ceil(az / st) * st; z <= bz + 1e-9; z += st) {
          ...
          out.push(
            node('path', {
              ops: [
                { op: 'M', x: base[0], y: base[1], depth: base[2] },
                { op: 'L', x: tip[0], y: tip[1], depth: tip[2] },
              ],
              color: col,
              stroke: c.stroke || 1.6,
              z: depthZ(base[2]),
          ...
          out.push(
            node('polygon', {
              pts: [
                [tip[0], tip[1]],
                [tip[0] - ux * head + px * head * 0.5, tip[1] - uy * head + py * head * 0.5],
                [tip[0] - ux * head - px * head * 0.5, tip[1] - uy * head - py * head * 0.5],
              ],
              closed: true,
              fill: col,
...
export class Polyhedron extends Drawable {
  constructor(conf = {}) {
    super('polyhedron', { ...conf });
  }
  toIR(ctx) {
    const c = this._conf;
    const out = [];
    for (const [i, j] of c.edges || [])
      out.push(polyline([project3(ctx, c.vertices[i]), project3(ctx, c.vertices[j])], c, 0));
    for (const v of c.vertices || []) {
      const p = project3(ctx, v);
      out.push(
        node('point', { x: p[0], y: p[1], marker: 'dot', z: depthZ(p[2], 1), style: { color: c.color || '#000' } }),
      );
    }
    return out;
  }
...
export const polyhedron = {
  platonic(name) {
    return { circumradius: (R) => new Polyhedron(buildPoly(name, R)) };
  },
};
...
function buildPoly(name, R) {
  ...
  for (let i = 0; i < verts.length; i++)
    for (let j = i + 1; j < verts.length; j++) minD = Math.min(minD, dist(verts[i], verts[j]));
  const edges = [];
  for (let i = 0; i < verts.length; i++)
    for (let j = i + 1; j < verts.length; j++) if (Math.abs(dist(verts[i], verts[j]) - minD) < 1e-6) edges.push([i, j]);
