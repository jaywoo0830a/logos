...
export class Parabola extends Drawable {
  constructor(conf = {}) { super('parabola', { ...conf }); }
  ...
  params() {
    ...
    const vx = F[0], vy = (F[1] + yDir) / 2; // 꼭짓점
  ...
  toIR(ctx) {
    ...
    for (let i = 0; i <= n; i++) {
      ...
      const y = yAt(x);
      if (!Number.isFinite(y)) { start = false; continue; }
      ops.push({ op: start ? 'L' : 'M', x, y });
      start = true;
    }
    const out = [node('path', { ops, color: c.color, stroke: c.stroke, dash: c.dash, opacity: c.opacity, style: pickStyleP(c) })];
    if (c.label) {
      const xl = vx + h * 0.7, yl = yAt(xl);
      out.push(node('text', { x: xl, y: yl - 0.3, text: String(c.label.toLatex ? c.label.toLatex() : c.label), anchor: 'start', math: !!c.label.toLatex, color: c.color }));
    }
    return out;
  }
}
...
export const parabola = {
  focus(F) { return new ParabolaBuilder(F); },
  standard(p) {
...
class ParabolaBuilder {
  ...
  directrix(d) { return new Parabola({ focus: this._F, directrix: d }); }
}
