...
export function cmapColor(name, t) {
  ...
  const a = hexToRgb(stops[i]),
    b = hexToRgb(stops[i + 1]);
  return rgbToHex(a[0] + (b[0] - a[0]) * f, a[1] + (b[1] - a[1]) * f, a[2] + (b[2] - a[2]) * f);
}
/** 밝기 배율 k 로 명암 조절 */
export function shade(hex, k) {
  const [r, g, b] = hexToRgb(hex);
  const cl = (x) => Math.max(0, Math.min(255, x * k));
  return rgbToHex(cl(r), cl(g), cl(b));
}

const LIGHT = (() => {
  const v = [0.45, -0.6, 0.85];
...
export class Curve3 extends Drawable {
  constructor(conf = {}) {
    super('curve3', { kind: 'parametric', domain: [0, 1], n: 200, ...conf });
  }
  on(domain) {
    return this.set({ domain });
  }
  label(t, off) {
    return this.set({ label: t, labelOff: off });
  }
  get vertices() {
    return this._sample().map((v) => point(v[0], v[1], v[2] ?? 0));
  }
  _sample() {
    const c = this._conf;
    ...
    const n = c.n || 200,
      out = [];
    for (let i = 0; i <= n; i++) {
      const v = c.fn(a + ((b - a) * i) / n);
      if (v && Number.isFinite(v[0])) out.push([v[0], v[1], v[2] ?? 0]);
    }
    return out;
  }
  toIR(ctx) {
...
export const curve3 = {
  parametric(f) {
    return new Curve3({ kind: 'parametric', fn: f });
  },
  through(pts) {
...
export class Arrow3 extends Drawable {
  constructor(conf = {}) {
    super('arrow3', { head: 9, ...conf });
  }
  to(P) {
    return this.set({ to: asCoords(P) });
  }
  label(t, off) {
    return this.set({ label: t, labelOff: off });
  }
  ...
  ratio(r) {
    return this.set({ ratio: r });
  }
  get vertices() {
    return [this._conf.from, this._conf.to].map((v) => point(...v));
  }
  toIR(ctx) {
    const c = this._conf;
    const W = asCoords(c.from),
      V = asCoords(c.to);
    const A = project3(ctx, W),
      B = project3(ctx, V);
    ...
    const out = [
      node('path', {
        ops: [
          { op: 'M', x: A[0], y: A[1], depth: A[2] },
          { op: 'L', x: bx, y: by, depth: B[2] },
        ],
        color: col,
        stroke: w,
        dash: c.dash,
...
export class ParamSurface extends Drawable {
  constructor(conf = {}) {
    super('surface', { ur: [0, 1], vr: [0, 1], nu: 24, nv: 24, mode: 'wire', opacity: 0.95, ...conf });
  }
  on(ur, vr) {
    return this.set({ ur, vr });
  }
  wire(nu, nv) {
    return this.set({ mode: 'wire', nu: nu ?? this._conf.nu, nv: nv ?? this._conf.nv });
  }
  solid(nu, nv) {
    return this.set({ mode: 'solid', nu: nu ?? this._conf.nu, nv: nv ?? this._conf.nv });
  }
  cmap(name) {
  ...
  get vertices() {
    ...
    for (const u of [u0, (u0 + u1) / 2, u1])
      for (const v of [v0, (v0 + v1) / 2, v1]) {
        const w = c.fn(u, v);
        if (w && w.every(Number.isFinite)) out.push(point(w[0], w[1], w[2] ?? 0));
      }
    return out;
  }
  toIR(ctx) {
    ...
    const P = (i, j) => {
      const w = W(i, j);
      return w && w.every(Number.isFinite) ? project3(ctx, w) : null;
    };
    ...
    if (c.mode === 'solid') {
      ...
      const line = (pts) => {
        const l = polyline3(pts, { color: c.color || base, stroke, opacity: c.opacity }, 0, c.hidden !== false);
        if (l) out.push(l);
      };
...
export const quadrics = {
  ...
  plane(f, xr, yr) {
    return surfaceParam((u, v) => [u, v, f(u, v)]).on(xr, yr);
  },
  /** 타원체 x²/a² + y²/b² + z²/c² = 1 */
  ...
  /** 구 (반지름 r) */
  ball(r, center = [0, 0, 0]) {
    return quadrics.ellipsoid(r, r, r, center);
  },
  ...
  cone(k, center = [0, 0, 0], vr = [-2, 2]) {
    return surfaceParam((u, v) => [center[0] + k * v * Math.cos(u), center[1] + k * v * Math.sin(u), center[2] + v]).on(
      [0, TAU],
      vr,
    );
  },
  ...
  cylinder(r, center = [0, 0, 0], zr = [-2, 2]) {
    return surfaceParam((u, v) => [center[0] + r * Math.cos(u), center[1] + r * Math.sin(u), center[2] + v]).on(
      [0, TAU],
      zr,
    );
  },
...
export function frame3(xlim, ylim, zlim) {
  return new Frame3({ xlim, ylim, zlim });
}
