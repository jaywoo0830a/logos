...
export class Polygon extends Drawable {
  constructor(conf = {}) { super('polygon', { ...conf, vertices: [...conf.vertices] }); }
  get vertices() { return this._conf.vertices; }
...
export function triangle(A, B, C) { return new Polygon({ vertices: [A, B, C], fill: 'none' }); }
...
function equalTriangle(B, C) {
  ...
  const mx = (B.coords[0] + C.coords[0]) / 2, my = (B.coords[1] + C.coords[1]) / 2;
  const h = d * Math.sqrt(3) / 2;
  const A = _point(mx, my + h);
  return {
    above() { return new Polygon({ vertices: [A, B, C], fill: 'none' }).O ? null : new Polygon({ vertices: [A, B, C], fill: 'none' }); },
  };
}

export function quad(A, B, C, D) { return new Polygon({ vertices: [A, B, C, D], fill: 'none' }); }
