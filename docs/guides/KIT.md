...
가장 잦았던 사고는 _패널이 서로 겹치는_ 경우였습니다.
...
const figs = [
  [
    ...
    () =>
      kit
        .plot2d([-2, 2], [-2, 2], { equal: true })
        .title('Unit circle')
        .add(circle.center(point(0, 0)).radius(1).color(kit.palette.blue)),
    '단위원',
  ],
];
...
scene()
  ...
  .axes()
  .add(
    arrowAt([0, 0], P(z)), // z 를 벡터로
    circle.center(point(0, 0)).radius(z.abs), // |z| 원
    annotate.angle({ from: [1, 0], vertex: [0, 0], to: P(z) }).arc({ radius: 0.6 }),
  ); // arg z
```
