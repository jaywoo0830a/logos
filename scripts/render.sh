...
#
# 렌더는 **항상 도커 컨테이너**에서 실행한다 — 호스트엔 bash+docker 만 있으면 되고,
# Node/폰트/래스터라이저는 이미지 안에서 해결되어 결과가 일정하다.
# 리눅스 전용.
#
# 사용법
...
RECURSIVE=0; QUIET=0; BUILD=0; DO_INSTALL=0; SERVE=0; SERVE_PORT=18080; LIVE_PKG=0
CLI_OPTS=()
...
# ── 결과 (호스트 Node 불필요 — manifest 를 sed 로 읽는다) ──
step "[4/4] 산출물"
if [[ -f "$OUT_ABS/manifest.json" ]]; then
  _ok="$(sed -nE 's/.*"ok"[[:space:]]*:[[:space:]]*([0-9]+).*/\1/p' "$OUT_ABS/manifest.json" | head -1)"
  _fail="$(sed -nE 's/.*"fail"[[:space:]]*:[[:space:]]*([0-9]+).*/\1/p' "$OUT_ABS/manifest.json" | head -1)"
  log "  ${_ok:-?}개 figure · 실패 ${_fail:-?}"
fi
log ""
ok "출력 폴더: $OUT_ABS"
