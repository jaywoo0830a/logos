...
export function runPython(code, { timeout = 30000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const t = setTimeout(() => {
      child.kill('SIGKILL');
      reject(new Error('python3 timeout'));
    }, timeout);
...
export class SageAdapter {
  ...
  async call(tool, args) {
    ...
    throw new Error(
      'SageAdapter: cmd 는 mcp/rest — SageMath 배포 후 세션을 주입하세요. 폴백으로 SymPyAdapter 가 동작합니다.',
    );
  }
  async available() {
    return false;
  ...
  diff(latex, v = 'x') {
    return this.call('differentiate_expression', { expression: latex, variable: v });
  }
  integrate(latex, o = {}) {
    return this.call('integrate_expression', { expression: latex, variable: o.var ?? 'x', from: o.from, to: o.to });
  }
  solve(latex, v = 'x') {
    return this.call('solve_equation', { equation: latex, variable: v });
  }
  simplify(latex) {
    return this.call('sample_expression', { expression: latex });
  }
}
...
export class SymPyAdapter {
  static async detect() {
    try {
      return !!(await runPython('import sympy; print(sympy.__version__)'));
    } catch {
      return false;
    }
  }
  async available() {
    return SymPyAdapter.detect();
  }
  ...
  async simplify(latex) {
    return this._run(`simplify(parse_expr(${QL(latex)}))`);
  }
  ...
  async series(latex, v, at, order) {
    return this._run(
      `series(parse_expr(${QL(latex)}), ${SYM(v)}, ${JSON.stringify(at)}, ${JSON.stringify(order)}).removeO()`,
    );
  }
...
export function withExternalSymbolic(SymClass) {
  return class ExtSym extends SymClass {
    async diff(v = 'x') {
      const r = await defaultAdapter.diff(this.latex, v);
      return new ExtSym(r);
    }
    async integrate(o) {
      const r = await defaultAdapter.integrate(this.latex, { var: 'x', ...o });
      return new ExtSym(r);
    }
    async solve(v = 'x') {
    ...
    async simplify2() {
      const r = await defaultAdapter.simplify(this.latex);
      return new ExtSym(r);
    }
  };
}
