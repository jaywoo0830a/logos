...
```js
if (failN) throw new AggregateError(failed.map((f) => new Error(f.error, { cause: f.src })), `${failN}개 figure 실패`);
```
...
export const toPoint = (v, ...rest) =>
  v?.coords ? v
  : Array.isArray(v) ? point(...v, ...rest)
  : (v && typeof v === 'object' && 'x' in v)
    ? ('z' in v ? point(v.x, v.y, v.z) : point(v.x, v.y))
  : point(v, ...rest);
```
