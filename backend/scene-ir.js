...
export class SceneIR {
  ...
  [Symbol.iterator]() {
    return this.o.nodes[Symbol.iterator]();
  }

  _map() {
    const { world, size, equal } = this.o;
    const [W, H] = size;
    const xr = world.xmax - world.xmin,
      yr = world.ymax - world.ymin;
    let ox = 0,
      oy = 0,
      sx,
      sy,
    ...
    if (equal) {
      ...
      oy = (H - yr * scale) / 2;
      sx = scale;
      sy = scale; // equal: 양축 동일 스케일
    } else {
      sx = W / xr;
      sy = H / yr;
      scale = Math.min(sx, sy);
    }
    const map = (x, y) => [ox + (x - world.xmin) * sx, oy + (world.ymax - y) * sy];
  ...
  toSVG(opts = {}) {
    ...
    const svg = emitSVG(nodes, {
      ...
      axisColor: td.axisColor,
      gridColor: td.gridColor,
  ...
  toJSXGraphHTML(opts = {}) {
    return buildJSXGraphHTML(this.o.nodes, opts);
  }
...
export function panels(figures, opts = {}) {
  ...
  return {
    ...
    toTikZ() {
      return figures.map((f) => f.toTikZ({ standalone: true })).join('\n\n');
    },
    async toPNG(o = {}) {
      let Resvg;
      try {
