...
test('D1 삼각형 채우기', () => {
  const svg = scene()
    .equal()
    .add(triangle(point(0, 0), point(4, 0), point(2, 3)).fill('#eef'))
    .compile()
    .toSVG();
...
test('F1 극좌표 격자', () => {
  ...
  test('G1 각도: arc + double', () => {
    ...
    const svg = scene()
      .equal()
      .add(triangle(A, B, C), annotate.angle(A, B, C).arc({ radius: 1.0, double: true }).label('α').degrees())
      .compile()
      .toSVG();
  ...
  test('I1 dash 패턴', () => {
    const svg = scene()
      .equal()
      .add(line.slopeIntercept(1, 0).dash([5, 3]))
      .compile()
      .toSVG();
  ...
  test('I2 투명도', () => {
    const svg = scene()
      .equal()
      .add(circle.center(point(0, 0)).radius(1).opacity(0.3))
      .compile()
      .toSVG();
  ...
  test('J5 유니코드/그리스/한글 라벨', () => {
    const svg = scene().axes().add(point(0, 0).dot().label('α'), point(1, 0).dot().label('점 A')).compile().toSVG();
  ...
  test('K1 삼각형 내심', () => {
    ...
    const svg = scene()
      ...
      .equal()
      .axes()
      .add(tri, A, B, C, circle.inscribed(tri), segment(A, I))
      .compile()
      .toSVG();
  ...
  test('K2 원과 접선', () => {
    ...
    const svg = scene()
      ...
      .equal()
      .axes()
      .add(C.stroke(2), O, P, T, line.through(P, T).color('#c00'))
      .compile()
      .toSVG();
