...
const ticks = (svg) =>
  [...svg.matchAll(/<text[^>]*\by="(-?[\d.]+)"[^>]*font-size="12"[^>]*>([^<]+)<\/text>/g)].map(
    (m) => `${m[2]}@${m[1]}`,
  );
...
test('axes({ y: { decimals } }) 로 자릿수를, { y: { format } } 로 문자열을 재정의한다', () => {
  const d = scene()
    .axes({ y: { decimals: 4 } })
    .view([0, 1], [0.5001, 0.5009])
    .compile()
    .toSVG();
  assert.ok(
    ticks(d).some((t) => /^0\.500\d@/.test(t)),
    `decimals:4 → ${ticks(d).join(' ')}`,
  );

  const f = scene()
    .axes({ y: { format: (v) => `${Math.round(v * 100)}%` } })
    .view([0, 1], [0, 1])
    .compile()
    .toSVG();
  assert.ok(
    ticks(f).some((t) => /^\d+%@/.test(t)),
    `format 훅 → ${ticks(f).join(' ')}`,
  );
});
