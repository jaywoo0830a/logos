...
export const figures = {
  ...
  A3_circle_equal: () =>
    scene()
      .equal()
      .axes()
      .add(circle.center(point(0, 0)).radius(3).fill('#dfe7fb').opacity(0.35).stroke(2)),
  A4_styles: () =>
    scene()
      .axes()
      .add(point(0, 0).dot().color('#e11'), point(1, 1).dot().color('#0a0').stroke(3), point(3, 3).dot().opacity(0.4)),
  B1_fn: () =>
    scene()
      .view([-3, 3], [-1, 9])
      .equal()
  ...
  C1_ellipse: () =>
    scene()
      .equal()
      .axes()
      .add(ellipse.center(point(0, 0)).semi(3, 2).stroke(2).color('#7c3aed')),
  C2_parabola: () =>
    scene()
      .equal()
      .axes()
      .add(
        ...
        line.horizontal(-1).color('#888'),
      ),
  C3_hyperbola: () =>
    scene()
      .equal()
      .axes()
      .add(hyperbola.center(point(0, 0)).semi(1.5, 1).color('#0f766e').stroke(2)),
  D1_triangle: () =>
    scene()
      .equal()
      .axes()
      .add(triangle(point(0, 0), point(4, 0), point(2, 3)).fill('#a5d8ff').opacity(0.5).stroke(2).color('#1971c2')),
  D3_riemann: () =>
    scene()
      .view([-0.5, 2.5], [-0.5, 5])
      .axes()
  ...
  G4_integral: () =>
    scene()
      ...
      .add(
        ...
        region
          .below(curve.fn(Math.sin).on([0, pi]))
          .fill('#4dabf7')
          .opacity(0.4),
        annotate
          .integral(tex`\sin x`)
  ...
  K1_incenter: () =>
    scene()
      ...
      .add(
        ...
        circle
          .inscribed(triangle(point(0, 0), point(5, 0), point(1.5, 4)))
          .color('#e11')
          .dash([4, 3]),
      ),
  K2_tangent: () =>
    ...
    scene()
      ...
      .add(
        curve
          ...
          .label(tex`f(x)=x^2-1`),
        line
          .tangent(curve.fn(tex`x^{2} - 1`).on([-3, 3]))
          .at(1)
          .dash([5, 3])
          .color('#666'),
        point(1, 0)
    ...
    scene()
      ...
      .add(
        curve
          .fn((x) => x * x)
          .on([-1.5, 1.5])
          .clip(region.between(line.horizontal(-1), line.horizontal(1))),
      ),
...
export const scenes = Object.fromEntries(
  Object.entries(figures).map(([name, fig]) => [
    ...
    () =>
      fig()
        .compile()
        .toSVG(TEXT_MATH.has(name) ? { math: 'text' } : undefined),
  ]),
);
