...
import {
  ...
  region, annotate, point as ppoint,
  vector, regular, transform, surface, vectorField, tex, pi,
} from '../index.js';
...
test('#1 함수 그래프 + 정의역 라벨', () => {
  const svg = scene().view([-3.5, 3.5], [-1, 9]).axes().add(curve.fn((x) => x * x - 1).on([-3, 3]).label(tex`f(x) = x^2 - 1`)).compile().toSVG();
  assert.ok(/<path d="M/.test(svg), '곡선');
  assert.ok(svg.includes('<math') || svg.includes('<foreignObject'), '수식 라벨');
});

test('#2 수직점근선 + 극한 (1/(x-1))', () => {
  const svg = scene().view([-4, 5], [-6, 8]).axes().add(
...
test('#4 좌·우극한 (열린 점)', () => {
  const svg = scene().view([-2.5, 2.5], [-1.5, 1.5]).axes().add(
...
test('#10 정적분 아래 영역 + 라벨', () => {
  const svg = scene().view([-0.5, Math.PI + 0.5], [-0.5, 1.5]).axes().add(
...
test('#11 두 곡선 사이 영역', () => {
  ...
  const svg = scene().view([-0.2, 1.2], [-0.2, 1.2]).equal().axes().add(F, G, region.between(F, G).on([0, 1]).fill('steelblue').opacity(0.4)).compile().toSVG();
  assert.ok(svg.includes('steelblue'), '사이 영역');
});

test('#12 회전체 (disk)', () => {
  const svg = scene().dim(3).camera({ position: [6, -6, 4] }).add(
    surface.revolution(curve.fn((x) => Math.sqrt(x)).on([0, 4])).about(line.horizontal(0)).opacity(0.8),
  ).compile().toSVG();
...
test('#14 호 길이 (심볼릭 적분 캡션)', () => {
  const svg = scene().view([-0.2, 1.2], [-0.2, 1.2]).equal().add(
    curve.fn((x) => x * x).on([0, 1]).stroke(2),
    annotate.caption(tex`L = \int_0^1 \sqrt{1 + 4x^2}\,dx \approx 1.4789`),
  ).compile().toSVG();
  assert.ok(svg.includes('<math'), '호 길이 수식');
});
// ── C. 삼각형 기하 ───────────────────────────────
const tA = () => point(0, 0), tB = () => point(4, 0), tC = () => point(1.5, 3);

test('#17 내접원 + 내심', () => {
  const A = tA(), B = tB(), C = tC();
  const tri = triangle(A, B, C);
  const I = point.incenter(tri);
  assert.ok(I.coords[0] > 0 && I.coords[0] < 4, '내심 내부');
  const svg = scene().view([-1, 5], [-1, 4]).equal().add(tri, circle.inscribed(tri).color('#e11'), I.dot().label('I')).compile().toSVG();
...
test('#18 외접원 + 외심 (OA≈OB≈OC)', () => {
  ...
  const svg = scene().view([-3, 7], [-3, 5]).equal().add(circle.through(A, B, C).color('steelblue'), O.dot().label('O'), segment(O, A).dash([2, 2])).compile().toSVG();
...
test('#19 무게중심 + 중선', () => {
  ...
  const svg = scene().view([-1, 5], [-1, 4]).equal().add(
    G.dot().label('G'),
...
test('#23 닮음 각 호 (double)', () => {
  ...
  const svg = scene().equal().add(
    annotate.angle(A, B, C).arc({ radius: 1, double: true }).label('β'),
    annotate.angle(C, A, B).arc().label('α'),
  ).compile().toSVG();
...
test('#25 원주각=2×중심각', () => {
  const O = point(0, 0);
  const A = point.polar(2, Math.PI / 4), B = point.polar(2, 3 * Math.PI / 4);
  const svg = scene().view([-3, 3], [-3, 3]).equal().add(
    circle.center(O).radius(2), O.dot(), A.dot(), B.dot(),
...
test('#16 급수 부분합 수렴', () => {
  const pts = Array.from({ length: 10 }, (_, n) => {
    let s = 0; for (let k = 1; k <= n + 1; k++) s += 1 / (k * k);
    return point(n + 1, s).dot();
  });
  const svg = scene().view([0, 12], [0, 2]).axes().add(...pts, line.horizontal(Math.PI ** 2 / 6).dash([4, 3]).label(tex`\pi^2/6`)).compile().toSVG();
  assert.ok(count(svg, 'circle') >= 9, '부분합 점');
});

// ── E. 삼각함수 ──────────────────────────────────
test('#29 단위원 + sin/cos', () => {
  const O = point(0, 0), P = point.polar(1, Math.PI / 6);
  const svg = scene().view([-1.5, 1.5], [-1.5, 1.5]).equal().add(
    circle.center(O).radius(1), O.dot(), P.dot().label(tex`(\cos\theta, \sin\theta)`),
    segment(O, P), segment(P, point(Math.cos(Math.PI / 6), 0)).dash([2, 2]),
    annotate.angle(point(1, 0), O, P).arc().label(tex`\theta`),
  ).compile().toSVG();
...
test('#30 사인/코사인 위상', () => {
  const svg = scene().view([-2 * Math.PI - 0.5, 2 * Math.PI + 0.5], [-1.5, 1.5]).axes().add(
...
test('#31 탄젠트 + 점근선', () => {
  const svg = scene().view([-Math.PI / 2 - 0.5, Math.PI / 2 + 0.5], [-5, 5]).axes().add(
...
test('#33 삼각함수 합성 (동일 곡선)', () => {
  ...
  let maxDiff = 0;
  for (let i = 0; i <= 200; i++) { const x = (-4 + 8 * i / 200); maxDiff = Math.max(maxDiff, Math.abs(c(x) - r(x))); }
  assert.ok(maxDiff < 1e-6, `합성차 최대 ${maxDiff.toExponential(3)}`);
});

// ── F. 극좌표·매개변수 ───────────────────────────
test('#34 카디오이드', () => {
  const svg = scene().equal().polarGrid().add(curve.polar((t) => 1 + Math.cos(t)).on([0, 2 * Math.PI])).compile().toSVG();
...
test('#36 리마송 (내/외 루프', () => {
  const svg = scene().equal().add(curve.polar((t) => 1 + 2 * Math.cos(t)).on([0, 2 * Math.PI])).compile().toSVG();
  assert.ok(count(svg, 'path') >= 1, '리마송');
});

test('#38 리사주 3:4', () => {
  const svg = scene().equal().add(curve.parametric((t) => [Math.sin(3 * t), Math.sin(4 * t)]).on([0, 2 * Math.PI])).compile().toSVG();
...
test('#39 벡터 합 (평행사변형)', () => {
  ...
  const svg = scene().view([-1, 4], [-1, 4]).equal().axes().add(
...
test('#41 고유벡터', () => {
  const svg = scene().view([-1, 2], [-1, 2]).axes().add(
...
test('#44 정규분포 + 음영 (68%)', () => {
  ...
  const svg = scene().view([-4, 4], [-0.05, 0.5]).axes().add(
    curve.fn(gauss).on([-4, 4]).color('crimson'),
    region.below(curve.fn(gauss).on([-1, 1])).fill('steelblue').opacity(0.4),
...
test('#48 기울기장 vectorField', () => {
  const svg = scene().view([-3, 3], [-3, 3]).axes().add(vectorField((x, y) => [1, x + y])).compile().toSVG();
