...
test('B6 여러 곡선: z-order (나중 add 위에)', () => {
  const svg = scene()
    ...
    .compile()
    .toSVG();
  const ic = svg.indexOf('crimson'),
    is = svg.indexOf('steelblue');
...
test('C2 포물선: 초점-준선 거리 등식', () => {
  ...
  const svg = scene()
    .equal()
    .axes()
    .add(P.color('crimson').stroke(2), F, d, point(0, 0).dot().label('V'))
    .compile()
    .toSVG();
...
test('C3 쌍곡선: 좌우 두 branch', () => {
  const svg = scene()
    ...
    .add(
      ...
      line.slopeIntercept(2 / 3, 0).dash([4, 3]),
      line.slopeIntercept(-2 / 3, 0).dash([4, 3]),
    )
    .compile()
    .toSVG();
