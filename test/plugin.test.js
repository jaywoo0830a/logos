...
import {
  scene, point, circle, segment, curve, use, plugins, plugin, PluginError, Drawable,
  ray as rayStub, arc as arcStub,
} from '../index.js';
...
const mini = {
  ...
  install(api, opts = {}) {
    ...
    api.extend('drawable', { mark(name) { return this.set({ tag: name }); } }); // ④-b 명령형
...
test('use(): 설치·멱등·조회(list/has/info)', () => {
  ...
  } finally { plugin.uninstall('mini-test'); }
  assert.ok(!plugin.has('mini-test'));
});
...
test('define(): 새 빌더가 즉시 살아난다 — index.js 스텁(ray)까지', () => {
  ...
  } finally { plugin.uninstall('geometry-extras'); }
});
...
test('node(): 새 IR 노드가 두 백엔드로 나간다 (SVG pattern + TikZ pattern)', () => {
  use(geometryExtras, { watermark: false });
  try {
    const fig = scene().view([-3, 3], [-3, 3]).equal().add(
      new Hatched(-1, -1, 2, 1.4).hatch({ gap: 6 }).text('A').color('#8b0000'),
    );
    const svg = svgOf(fig);
...
test("hook('svg') / hook('ir'): 파이프라인 후처리", () => {
  ...
  const irHook = {
    name: 'ir-hook',
    install(api) {
      api.hook('ir', (nodes) => nodes.concat([{
        kind: 'point', data: { x: 0, y: 0, marker: 'dot', size: 6, style: {} }, children: [],
      }]));
    },
  };
  try {
  ...
  } finally { plugin.uninstall('ir-hook'); }
});
...
test('around(): 데이터 반환 메서드는 값을 훼손하지 않는다 (체이닝 강제 없음)', () => {
  const raw = {
    ...
    install(api) {
      ...
      api.around('point', 'size', function (orig, n) { return this.set({ size: n * 2 }); }); // 체이닝
  ...
  try {
    ...
    assert.deepEqual(curve.fn((x) => x * x).on([0, 1]).eval(0.5).cart, [0.5, 0.25], 'eval 반환값 유지');
  } finally { plugin.uninstall('raw-around'); }
  assert.equal(point(1, 2).dim(), 2, 'uninstall 로 원복');
});

test('미등록 이름: 코어 수정 대신 플러그인 등록을 안내한다', () => {
  let err = null;
  try { rayStub(point(0, 0), point(1, 1)); } catch (e) { err = e; }
...
test('형식 오류·이름 충돌은 PluginError + 부분 설치 롤백', () => {
  ...
  try {
    ...
    assert.throws(
      () => use({ name: 'dup', install(api) { api.define('ray', () => {}); } }),
      PluginError,
    );
    assert.throws(() => use({ name: 'bad' }), PluginError, 'install 없음');
    assert.throws(
      () => use({ name: 'bad2', install(api) { api.extend('nope', { x() {} }); } }),
      PluginError, '알 수 없는 대상',
    );
    assert.ok(!plugin.has('bad2'), '실패한 설치는 목록에 남지 않는다');
    assert.equal(typeof plugins.ray, 'function', '기존 설치는 그대로');
  } finally { plugin.uninstall('dup'); plugin.uninstall('geometry-extras'); plugin.reset(); }
});
...
test('실제 플러그인: 체이닝 메서드 3종이 렌더에 반영된다', () => {
  ...
  try {
    ...
    const svg = svgOf(scene().view([-3, 3], [-3, 3]).equal()
    ...
    const figOf = (deg) => scene().view([-3, 3], [-3, 3]).equal()
      .add(segment(point(1, 0), point(2, 0)).tilt(deg));
    const node0 = figOf(0).compile().o.nodes.find((n) => n.kind === 'path');
    const node90 = figOf(90).compile().o.nodes.find((n) => n.kind === 'path');
