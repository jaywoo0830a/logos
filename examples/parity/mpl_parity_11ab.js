...
const SIN = '#e74c3c',
  COS = '#2980b9',
  TAN = '#27ae60'; // C['sin'|'cos'|'tan']
const CSC = '#e67e22',
  SEC = '#9b59b6',
  COT = '#1abc9c'; // C['csc'|'sec'|'cot']
const ASYMP = '#cccccc',
  HL = '#f1c40f',
  CIRC = '#2c3e50';
const GRAY = '#999999',
...
const arcAt = (from, to, r, o) => style(annotate.angle({ from, vertex: [0, 0], to }).arc({ radius: r }), o);
...
const piTick = (k) =>
  Number.isInteger(k) ? (k === 0 ? '0' : k === 1 ? 'π' : `${k}π`) : k === 0.5 ? 'π/2' : `${k * 2}π/2`;
...
function radianDefinition() {
  const r = 1.2,
    th = 1.0; // 1 rad (라디안)
  const p1 = onCircle(r, 0),
    p2 = [r * Math.cos(th), r * Math.sin(th)];
  return s2([-1.8, 1.8], [-1.8, 1.8], OFF)
    .title('1 Radian')
    .add(
...
function specialAngles() {
  ...
  const sp = [
    [0, '0', '(1, 0)', 0.08, 0.08],
    [30, 'π/6', '(√3/2, 1/2)', 0.08, 0.08],
    [45, 'π/4', '(√2/2, √2/2)', 0.18, 0.32],
    [60, 'π/3', '(1/2, √3/2)', 0.1, -0.04],
    [90, 'π/2', '(0, 1)', 0.0, 0.14],
  ];
  return s2([-1.6, 1.6], [-1.6, 1.6], OFF)
    ...
    .add(
      ...
      ...sp.flatMap(([d, lr, coord, ox, oy]) => {
        ...
        return [
          ...
          labelAt([p[0] + ox, p[1] + oy], `${lr}\n${coord}`, { font: 10, bold: true, box: BOX }),
        ];
      }),
    )
    .compile();
...
function referenceAnglesAstc() {
  const r = 1.2,
    p = onCircle(r, 150);
  const left = s2p([-1.5, 1.5], [-1.5, 1.5], TWO, { equal: true, ...OFF })
  ...
  const right = s2p([-1.6, 1.6], [-1.6, 1.6], TWO, { equal: true, ...OFF })
    .title('ASTC Signs')
    .add(
      ...
      ...[
        ...
        [-0.6, -0.6, 'QIII\nsin − cos −', CSC],
        [0.6, -0.6, 'QIV\nsin − cos +', COS],
      ].map(([x, y, t, c]) => labelAt([x, y], t, { color: c, font: 11, bold: true, anchor: 'middle', box: BOX })),
      ...[
        ['A', 1.35, 1.35],
        ['S', -1.35, 1.35],
        ['T', -1.35, -1.35],
...
function sinCosGraphs() {
  ...
  const circlePanel = (title, marker, dashColor, vertical) =>
    s2p([-1.5, 1.5], [-1.5, 1.5], CELL6, { equal: true, ...OFF })
      .title(title)
      .add(
        ...
        ...Array.from({ length: 13 }, (_, i) => i * 30).flatMap((d) => {
          ...
          const leg = vertical
            ? segAt([q[0], 0], q, { color: dashColor, stroke: 1, dash: [3, 3], opacity: 0.5 })
  ...
  const p3 = s2p([-pi, 4 * pi], [-1.5, 1.5], CELL6, {
    axes: { x: { label: 'θ', ticks: false }, y: { label: 'y' } },
    grid: { alpha: 0.3 },
  })
  ...
  const p4 = s2p([0, 1], [0, 1], CELL6, OFF).add(
    labelAt([0.1, 0.5], 'Period 2π\nAmplitude 1\nRange [−1, 1]', { font: 16, anchor: 'start', box: BOX }),
  );
...
function tanGraph() {
  const r = 1.2,
    ang = 35,
    tv = Math.tan(rad(ang));
  const left = s2p([-1.8, 1.8], [-1.8, 1.8], TWO, { equal: true, ...OFF })
    .title('tan θ = Slope')
    .add(
      circleAt([0, 0], r, { color: CIRC, stroke: 1.5 }),
      hlineAt(0, { color: GRAY, stroke: 0.5 }),
...
function cscSecCotGraphs() {
  const xr = [0.01, 2 * pi - 0.01],
    xlim = [0, 2 * pi];
  const mk = (title, f, color, breaks, asymptotes, xLabel = false) =>
    sw(xlim, [-6, 6], {
      ...
      .add(
        ...
        hlineAt(1, { color: GRAY, stroke: 0.5, dash: [2, 2] }),
        hlineAt(-1, { color: GRAY, stroke: 0.5, dash: [2, 2] }),
        ...Array.from({ length: 5 }, (_, n) => n / 2).map((k) => piTickAt(xlim, k, -5.15)),
      );
  return subplots(
    [
...
function unitCircleSinCosTan() {
  const r = 1.2,
    ...
    p = onCircle(1, ang),
    tv = Math.tan(rad(ang));
  return s2([-2, 2], [-2, 2], OFF)
    .title('sin, cos, tan on Unit Circle')
    .add(
      circleAt([0, 0], r, { color: CIRC, stroke: 2 }),
      hlineAt(0, { color: GRAY, stroke: 0.5 }),
      vlineAt(0, { color: GRAY, stroke: 0.5 }),
...
function unitCircleCscSecCot() {
  ...
  return s2([-1.5, 1.5], [-1.5, 1.5], OFF)
    ...
    .add(
      ...
      dotAt(sPt, SEC, 6),
      labelAt([sPt[0] * 0.45 + 0.1, sPt[1] * 0.45 - 0.16], 'sec', { color: SEC, font: 18, bold: true, box: BOX }), // mpl 대비: csc 라벨과 겹쳐 아래로
      segAt([0, 0], cPt, { color: CSC, stroke: 3 }), // csc: y=1 위
      dotAt(cPt, CSC, 6),
      labelAt([cPt[0] * 0.6 + 0.12, cPt[1] * 0.6 + 0.06], 'csc', { color: CSC, font: 18, bold: true, box: BOX }), // mpl 대비: sec 라벨과 겹쳐 위로
    ...
    stages.map(([t, f, note], i) =>
      sw([0, 2 * pi], [-3.5, 3.5], {
        ...
        .add(
          ...
          ...(i === 4 ? Array.from({ length: 5 }, (_, n) => n / 2).map((k) => piTickAt([0, 2 * pi], k, -3.3)) : []),
        ),
    ),
...
function arcsinGraph() {
  ...
  return s2(lim, lim, { axes: AX('x', 'y'), grid: { alpha: 0.3 } })
    .title('y = arcsin x')
    .add(
...
function arcsinComposition() {
  const xr = [-2 * pi, 2 * pi],
    yr = [-pi / 2 - 0.3, pi / 2 + 0.3];
  return s2p(xr, yr, [700, 420], { axes: { x: { label: 'θ', ticks: false }, y: { label: 'y' } }, grid: { alpha: 0.3 } })
    .title('arcsin(sin θ)')
    .add(
      polyAt(
        [
...
function unitCircleToSineUnwrap() {
  ...
  const right = s2p([0, 2 * pi], [-1.5, 1.5], TWO, {
    axes: { x: { label: 'θ', ticks: false }, y: { label: 'sin θ' } },
    grid: { alpha: 0.3 },
  })
    .title('Sine Wave')
    .add(
      ...
      hlineAt(0, { color: GRAY, stroke: 0.5 }),
      ...Array.from({ length: 5 }, (_, n) => n / 2).map((k) => piTickAt([0, 2 * pi], k, -1.3)),
    );
  return subplots([left, right], { cols: 2, tight: true });
}
...
function rightTriangle({ title, val, scale, mode, color }) {
  ...
  if (mode === 'arcsin') {
    opp = val * scale;
    adj = Math.sqrt(1 - val ** 2) * scale;
    h = scale;
  } else if (mode === 'arccos') {
    adj = val * scale;
    opp = Math.sqrt(1 - val ** 2) * scale;
    h = scale;
  } else {
    opp = val * scale;
    adj = scale;
    h = Math.sqrt(1 + val ** 2) * scale;
  }
  const pad = 0.4,
    mx = Math.max(Math.abs(adj), opp, h) + pad;
  const xr = adj < 0 ? [-mx, mx] : [-pad, mx],
    yr = [-pad, mx];
  const rs = 0.2 * (mx / 5),
    sgn = adj >= 0 ? -1 : 1; // 직각 표시
  const nx = -opp / h,
  ...
  return s2p(xr, yr, SQ, { equal: true, ...OFF })
    .title(title)
    .add(
      polyAt(
        [
      ...
      labelAt([adj / 2, -0.35], `${Math.round(Math.abs(adj))}`, { font: 17, anchor: 'middle', box: BOX }),
      labelAt([adj / 2 + nx * perp, opp / 2 + ny * perp], `${Math.round(h)}`, { font: 17, anchor: 'middle', box: BOX }),
...
function beatPatterns() {
  const xr = [0, 4 * pi];
  const y1 = (t) => Math.sin(2.5 * t),
    y2 = (t) => Math.sin(3 * t);
  const mk = (title, items, yr, xLabel = false) =>
    sw(xr, yr, {
      axes: { x: { label: xLabel, ticks: false }, y: { label: 'y' } },
      grid: { alpha: 0.2 },
    })
  ...
  return subplots(
    [
      ...
      mk(
        'Beat Pattern',
        [curveOf((t) => y1(t) + y2(t), xr, { color: TAN, stroke: 2 }), hlineAt(0, { color: GRAY, stroke: 0.5 })],
        [-2.5, 2.5],
      ),
      mk(
        'Envelope Revealed',
...
function trigEquationSolutions() {
  ...
  const rows = [
    ...
    ['cos x = −√3/2', Math.cos, -Math.sqrt(3) / 2, COS, (k) => [Math.acos(k), 2 * pi - Math.acos(k)], 2 * pi],
    ['tan x = −1', Math.tan, -1, TAN, (k) => [Math.atan(k)], pi],
  ];
  return subplots(
    rows.map(([title, f, k, color, sols, per]) => {
      ...
      const dots = sols(k).flatMap((s) =>
        [-1, 0, 1, 2]
          .map((n) => s + n * per)
          .filter((xs) => xs >= xr[0] && xs <= xr[1])
          .map((xs) => dotAt([xs, k], color, 4)),
      );
      return sw(xr, isTan ? [-5, 5] : [-2, 2], {
        axes: { x: { label: false, ticks: false }, y: { label: 'y' } },
        grid: { alpha: 0.3 },
      })
        .title(title)
        .add(
...
function weierstrassSubstitution() {
  const ang = 50,
    p = onCircle(1, ang),
    tv = Math.tan(rad(ang) / 2);
  return s2([-1.8, 2.5], [-1.8, 1.8], OFF)
    .title('t = tan(x/2)')
    .add(
      circleAt([0, 0], 1, { color: CIRC, stroke: 1.5 }),
      hlineAt(0, { color: GRAY, stroke: 0.5 }),
      ...
      dotAt([0, tv], TAN, 7),
      labelAt([0.08, tv + 0.12], `t = tan(x/2) = ${tv.toFixed(2)}`, { color: TAN, font: 13, bold: true, box: BOX }),
      arcAt([1, 0], p, 0.35, { color: HL, stroke: 2 }),
...
function trigInequalities() {
  const r = 1.2;
  const panel = (title, color, fills, extra) =>
    s2p([-1.5, 1.5], [-1.5, 1.5], THREE, { equal: true, ...OFF })
      .title(title)
      .add(
        circleAt([0, 0], r, { color: CIRC, stroke: 1.5 }),
        hlineAt(0, { color: GRAY, stroke: 0.5 }),
  ...
  return subplots(
    [
      panel('sin x > 1/2', SIN, [arcSamples(r, 30, 150)], [hlineAt(0.5, { color: SIN, stroke: 1.5, dash: [6, 4] })]),
      panel(
        'cos x ≤ −√2/2',
        COS,
        [arcSamples(r, 135, 225)],
        [vlineAt(-Math.SQRT2 / 2, { color: COS, stroke: 1.5, dash: [6, 4] })],
      ),
      panel(
        'tan x > 1',
        TAN,
        [arcSamples(r, 45, 89.5), arcSamples(r, 225, 269.5)],
        [
...
function eulerFormulaComplex() {
  ...
  return s2([-2.2, 2.2], [-2.2, 2.2], OFF)
    ...
    .add(
      ...
      labelAt([-0.12, p[1] / 2], 'sin θ', { color: SIN, font: 16, bold: true, rotate: 90, anchor: 'middle', box: BOX }),
      arcAt([1, 0], p, 0.35, { color: HL, stroke: 2 }),
...
function chebyshevPolynomials() {
  ...
  return s2p([-1.05, 1.05], [-1.3, 1.3], SQ, { axes: AX('x', 'T_n(x)'), grid: { alpha: 0.3 } })
    .title('Chebyshev T_n(x)')
    .add(
      ...cs.map((c, i) => curveOf((x) => Math.cos((i + 1) * Math.acos(x)), [-1, 1], { color: c, stroke: 2, n: 240 })),
      hlineAt(0, { color: GRAY, stroke: 0.5 }),
      vlineAt(0, { color: GRAY, stroke: 0.5 }),
      hlineAt(1, { color: GRAY, stroke: 0.5, dash: [2, 2] }),
      hlineAt(-1, { color: GRAY, stroke: 0.5, dash: [2, 2] }),
...
function identityFamilyTree() {
  ...
  const edges = [
    ...
    ['sum', 'prodsum'],
    ['double', 'chebyshev'],
    ['harmonic', 'weierstrass'],
  ];
  const at = (k) => nodes.find((n) => n[0] === k);
  return s2p([0, 10], [0, 7], [700, 480], OFF)
    .title('Identity Connections')
    .add(
      ...edges.map(([s, d]) => {
        const a = at(s),
          b = at(d);
        return arrowAt([a[1], a[2] - 0.2], [b[1], b[2] - 0.2], { color: GRAY, stroke: 1.5, bend: 0.1 });
      }),
      ...nodes.map(([, x, y, label, tc, bc]) =>
        labelAt([x, y], label, {
          color: tc,
          font: 12,
          bold: true,
          anchor: 'middle',
          box: { facecolor: bc, alpha: 0.9 },
        }),
      ),
    )
    .compile();
...
function lawOfSinesCosines() {
  ...
  const label = (at, txt, color) =>
    labelAt(at, txt, { color, font: color ? 16 : 18, bold: true, anchor: color ? 'middle' : undefined });
  const left = s2p([-0.8, 5.8], [-0.8, 4.2], TWO, { equal: true, ...OFF })
    .title('Law of Sines')
    .add(
      ...tri(
        [
      ...
      label([5.1, -0.3], 'B'),
      label([2.1, 3.8], 'C'),
      label([3.7, 1.75], 'a', SIN),
      label([0.7, 1.85], 'b', COS),
      label([2.5, -0.25], 'c', TAN),
    );
  const right = s2p([-0.8, 4.8], [-0.8, 3.6], TWO, { equal: true, ...OFF })
    ...
    .add(
      ...
      label([4.1, -0.3], 'B'),
      label([1.6, 3.2], 'C'),
      label([2.95, 1.4], 'a', SIN),
      label([0.45, 1.5], 'b', COS),
      label([2, -0.25], 'c', TAN),
...
const figsA = [
  ...
  [
    '11a16-cos-arcsin',
    () => rightTriangle({ title: 'cos(arcsin 3/5)', val: 3 / 5, scale: 5, mode: 'arcsin', color: SIN }),
    'cos(arcsin 3/5)',
  ],
  [
    '11a16-tan-arccos',
    () => rightTriangle({ title: 'tan(arccos −5/13)', val: -5 / 13, scale: 13, mode: 'arccos', color: COS }),
    'tan(arccos −5/13)',
  ],
  [
    '11a16-sin-arctan',
    () => rightTriangle({ title: 'sin(arctan 3/4)', val: 3 / 4, scale: 4, mode: 'arctan', color: TAN }),
    'sin(arctan 3/4)',
  ],
];
...
await saveFigures(figsA, {
  dir: OUT_A,
  index: true,
  title: 'logos · 11A — 삼각함수 (example5.py 재현)',
});
await saveFigures(figsB, {
  dir: OUT_B,
  index: true,
  title: 'logos · 11B — 삼각함수 심화 (example5.py 재현)',
});
