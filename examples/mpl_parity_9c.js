// example2.py (Session 9C, 3D 기하 그림들) 재현 — logos 만 사용.
// 실행: node examples/mpl_parity_9c.js → output/parity9c/*.svg + *.png
import { mkdirSync, writeFileSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import {
  scene, panels, point, line, segment, circle, curve, region, triangle, annotate, tex, pi, tau,
  arrow3, curve3, surfaceParam,
} from '../index.js';

const OUT = join(dirname(fileURLToPath(import.meta.url)), '..', 'output', 'parity9c');
mkdirSync(OUT, { recursive: true });

const B = '#0000ff', R = '#ff0000', G = '#008000', M = '#ff00ff', OR = '#ff8c00', PUR = '#800080';
const GRAY = '#808080', SKY = '#87CEEB', CORAL = '#F08080', DRED = '#8B0000', WHEAT = '#F5DEB3';

const PCELL3 = [480, 440];
// z축을 mplot3d 처럼 0.75 로 압축(box aspect [4,4,3]); 자동 3D축은 끄고 직접 그린다.
const base3 = ({ elev = 20, azim = -50, aspect = [1, 1, 0.75] } = {}) =>
  scene().size(PCELL3[0], PCELL3[1]).dim(3).camera({ elev, azim, aspect }).axes(false);
const axes3 = (L = 5, col = GRAY, w = 1) => [
  arrow3([0, 0, 0], [L, 0, 0]).color(col).stroke(w),
  arrow3([0, 0, 0], [0, L, 0]).color(col).stroke(w),
  arrow3([0, 0, 0], [0, 0, L]).color(col).stroke(w),
];
const poly3 = (pts, col, w = 1, dash) => { const c = curve3.through(pts).color(col).stroke(w); return dash ? c.dash(dash) : c; };
const srf = (fn, ur, vr) => surfaceParam(fn).on(ur, vr);
const planeZ = (f, xr, yr) => srf((u, v) => [u, v, f(u, v)], xr, yr);
const O3 = [0, 0, 0];
const ellipS = (a, b, c, C = O3) => srf((u, v) => [C[0] + a * Math.sin(v) * Math.cos(u), C[1] + b * Math.sin(v) * Math.sin(u), C[2] + c * Math.cos(v)], [0, tau], [0, pi]);
const ballS = (r, C = O3) => ellipS(r, r, r, C);
const hyper1S = (a, b, c, C = O3, vr = [-2, 2]) => srf((u, v) => [C[0] + a * Math.cosh(v) * Math.cos(u), C[1] + b * Math.cosh(v) * Math.sin(u), C[2] + c * Math.sinh(v)], [0, tau], vr);
const hyper2S = (a, b, c, C = O3, vr = [0.7, 2]) => srf((u, v) => [C[0] + a * Math.sinh(v) * Math.cos(u), C[1] + b * Math.sinh(v) * Math.sin(u), C[2] + c * Math.cosh(v)], [0, tau], vr);
const coneS = (k, C = O3, vr = [-2, 2]) => srf((u, v) => [C[0] + k * v * Math.cos(u), C[1] + k * v * Math.sin(u), C[2] + v], [0, tau], vr);
const cylS = (r, C = O3, zr = [-2, 2]) => srf((u, v) => [C[0] + r * Math.cos(u), C[1] + r * Math.sin(u), C[2] + v], [0, tau], zr);
// 보이지 않는 프레임(자동 프레이밍 제어 — xlim/ylim/zlim 대응)
const frame3 = (xr, yr, zr) => curve3.through([[xr[0], yr[0], zr[0]], [xr[1], yr[1], zr[1]]]).opacity(0);
const circ3 = (r, cz, C = O3) => curve3.parametric((t) => [C[0] + r * Math.cos(t), C[1] + r * Math.sin(t), C[2] + cz]).on([0, tau]);
// panels(): 컴파일된 SceneIR 만 받는다
const pfig = (figs, opts) => panels(figs.map((f) => (typeof f.compile === 'function' ? f.compile() : f)), { cell: PCELL3, ...opts });
const PCELL2 = [420, 400];
const pfig2 = (figs, opts) => panels(figs.map((f) => (typeof f.compile === 'function' ? f.compile() : f)), { cell: PCELL2, ...opts });
const s2 = (xr, yr) => scene().size(PCELL2[0], PCELL2[1]).view(xr, yr).equal().axes().grid({ alpha: 0.3 });

// ── 1. 3D Coordinate System ──
function coordSys3d() {
  return base3({ elev: 25, azim: -50 }).title('3D Coordinate System').add(
    ...
    poly3([[3, 2, 0], [3, 2, 4]], '#000', 0.8, [4, 3]).opacity(0.5),
    poly3([[3, 0, 0], [3, 2, 0]], '#000', 0.8, [4, 3]).opacity(0.5),
    poly3([[0, 0, 0], [3, 0, 0]], '#000', 0.8, [4, 3]).opacity(0.5),
...
function step3dCoords() {
  ...
  return pfig(pts.map((p, i) => {
    const last = p[p.length - 1];
    const shapes = [
      ...axes3(5, GRAY, 1.2),
      poly3([...paths[i], last], '#000', 2),
      point(...last).marker('circle').color(R).size(7),
    ];
    if (i >= 1) shapes.push(poly3([[3, 2, 0], [3, 2, 4]], '#000', 0.8, [4, 3]).opacity(0.4));
    return base3({ elev: 20, azim: -50 }).title(titles[i]).add(...shapes);
...
function vectorDotCross() {
  ...
  const left = scene().size(PCELL3[0], PCELL3[1]).view([-0.5, 4.5], [-0.5, 4.5]).equal().grid({ alpha: 0.3 }).axes()
  ...
  const right = base3({ elev: 20, azim: -60 }).title('Cross Product: Perpendicular')
    .add(
      ...axes3(3, GRAY, 0.8),
  ...
  return pfig([left, right], { cols: 2, title: 'Dot Product and Cross Product', tight: true });
}
...
function stepVectors() {
  ...
  const mk = (i) => {
    const shapes = [...axes3(5, GRAY, 0.8), arrow3([0, 0, 0], [3, 1, 2]).color(B).stroke(2).label('u=(3,1,2)')];
    if (i === 1) {
  ...
  return pfig([mk(0), mk(1), mk(2)], { cols: 3, title: 'Vectors in 3D — Step by Step', tight: true });
}

// ── 5. Plane 2x+3y−z=6 — Intercepts + Normal ──
function planeIntercept() {
  return base3({ elev: 22, azim: -55 }).title('Plane: 2x+3y−z=6 — Intercepts + Normal Vector').add(
    ...axes3(5, GRAY, 0.8),
    ...
    annotate.text(point(5.8, 0, 0)).label('x').font(12),
    annotate.text(point(0, 5.3, 0)).label('y').font(12),
    annotate.text(point(0, 0, 5.5)).label('z').font(12),
  ).compile();
}

// ── 6. Normal Vector ⊥ to Plane ──
function planeNormal() {
  const inPlane1 = [0, 1, 2, 3].map((t) => poly3([[t, 0, -6 + 2 * t], [t + 1.5, 0, -6 + 2 * (t + 1.5)]], '#000', 1.5).opacity(0.4));
  const inPlane2 = [0, 1, 2].map((t) => poly3([[t, 0, -6 + 2 * t], [t, 1.5, -6 + 2 * t - 4.5]], '#000', 1.5).opacity(0.4));
  const normals = [[1, 1, -1], [3, 0, 0], [0, 2, 0]].map((p) => arrow3(p, [p[0] + 2, p[1] + 3, p[2] - 1]).color(DRED).stroke(2.2).opacity(0.8));
  return base3({ elev: 18, azim: -50 }).title('Normal Vector ⊥ to Plane').add(
    ...axes3(5, GRAY, 0.8),
...
function stepPlane() {
  ...
  return pfig([0, 1, 2].map((i) => {
    const shapes = [...axes3(5, GRAY, 0.8), ...pts,
...
function pointPlaneDistance() {
  ...
  return base3({ elev: 22, azim: -55 }).title('Point-to-Plane Distance: D = |2x₀+3y₀+z₀−6|/√14').add(
    ...
    annotate.text(point(5.2, 0, 0)).label('x').font(12),
    annotate.text(point(0, 5.3, 0)).label('y').font(12),
    annotate.text(point(0, 0, 5.5)).label('z').font(12),
  ).compile();
}
...
function anglePlanes() {
  ...
  const p1 = base3({ elev: 25, azim: -50 }).title('Planes at 60°').add(
    ...axes3(3, GRAY, 0.7),
  ...
  const p2 = base3({ elev: 25, azim: -50 }).title('Perpendicular Planes').add(
    ...axes3(3, GRAY, 0.7),
    planeZ(() => 0, [-2, 2], [-2, 2]).solid(14, 14).color('#ADD8E6').opacity(0.4),
  ...
  return pfig([p1, p2], { cols: 2, title: 'Angle Between Planes = Angle Between Normals', tight: true });
}
...
function distanceParallelPlanes() {
  ...
  return base3({ elev: 15, azim: -55 }).title('Distance Between Parallel Planes  2x−y+2z=5 and 2x−y+2z=−7').add(
    ...axes3(4, GRAY, 0.7),
...
function sphereDetails() {
  ...
  return base3({ elev: 20, azim: -50 }).title('Sphere: (x−2)²+(y+3)²+(z−1)²=25').add(
    ...axes3(3, GRAY, 0.8),
    ballS(Rr, C).wire(40, 16).color(B).opacity(0.3),
    ...gc,
    point(...C).marker('circle').color(R).size(7),
    poly3([C, [C[0] + Rr, C[1], C[2]]], R, 2, [6, 4]),
...
function pointSphereDistance() {
  ...
  return pfig2([mk(10, 'P(10,0,0)', 'd=10−5=5', false), mk(2, 'P(2,0,0)', 'd=5−2=3', true)],
    { cols: 2, title: 'Point-to-Sphere Distance' });
}

// ── 13. Surface Height Map ──
function surfaceHeightMap() {
  return base3({ elev: 25, azim: -50 }).title('z = x² + y² — Height Map').add(
    ...axes3(4, GRAY, 0.8),
    planeZ((x, y) => x * x + y * y, [-2, 2], [-2, 2]).solid(28, 28).cmap('viridis').opacity(0.85),
    poly3([[1, 1, 0], [1, 1, 2]], R, 1.5, [6, 4]),
    poly3([[0, 0, 0], [1, 1, 0]], '#000', 0.8, [4, 3]).opacity(0.5),
...
function stepSurfaceBuild() {
  ...
  return pfig([0, 1, 2].map((i) => {
    const shapes = [...axes3(4, GRAY, 0.8), point(0, 0, 0).marker('circle').color(R).size(5)];
    shapes.push(i === 0 ? planeZ(f, [-2, 2], [-2, 2]).wire(20, 20).color(B).opacity(0.4)
      : planeZ(f, [-2, 2], [-2, 2]).solid(28, 28).cmap('viridis').opacity(0.75));
    if (i === 2) for (const cz of [1, 2, 3, 4]) shapes.push(circ3(Math.sqrt(cz), cz).color('#ffffff').stroke(1.5));
    return base3({ elev: 25, azim: -50 }).title(titles[i]).add(...shapes);
...
function domainRegions() {
  ...
  const d = s2([-1, 4], [-2, 3]).title('z=√x/(y−1)  x≥0, y≠1')
    .add(region.bar(0, 4, -2, 2).fill(PUR).opacity(0.15),
      ...
      line.horizontal(0).color(GRAY).stroke(0.5), line.vertical(0).color(GRAY).stroke(0.5));
  return pfig2([a, b, c, d], { cols: 2, title: 'Domain Regions in the xy-Plane' });
}
...
function levelCurvesToSurface() {
  ...
  return panels([bowl2d, bowl3d, sad2d, sad3d].map((f) => f.compile()), { cell: PCELL3, cols: 2, title: 'From Level Curves to 3D Surface' });
}
...
function contourSteepness() {
  ...
  return pfig2([steep, flat], { cols: 2, title: 'Contour Spacing Reveals Steepness' });
}
...
function stepLevelCurves() {
  ...
  const c = base3({ elev: 25, azim: -50 }).title('Step 3: Stack into 3D  Saddle emerges')
    .add(planeZ((x, y) => x * x - y * y, [-2, 2], [-2, 2]).solid(30, 30).cmap('coolwarm').opacity(0.8));
  return panels([a, b, c].map((f) => f.compile()), { cell: PCELL3, cols: 3, title: 'Level Curve Analysis — z=x²−y² (Saddle)', tight: true });
}
...
function quadricIdentification() {
  ...
  return pfig(specs.map(([t, list]) => base3({ elev: 20, azim: -50 }).title(t).add(...list.map((o) => o.color(B).opacity(0.5)))), { cols: 3, title: 'Quadric Surface Identification', tight: true });
}

// ── 21. Ellipsoid Details (1×2) ──
function ellipsoidDetails() {
  const a = 2, b = 3, c = 1;
  const left = base3({ elev: 20, azim: -50 }).title('Ellipsoid  x²/4+y²/9+z²=1')
    .add(...axes3(3.5, GRAY, 0.7), ellipS(a, b, c).wire(30, 20).color(B).opacity(0.4),
  ...
  const right = base3({ elev: 20, azim: -50 }).title('Three Orthogonal Cross-Sections').legend('upper right')
  ...
  return pfig([left, right], { cols: 2, title: 'Ellipsoid — The 3D Ellipse', tight: true });
}

// ── 22. Elliptic Paraboloid Details (1×2) ──
function paraboloidDetails() {
  const f = (x, y) => x * x + 2 * y * y;
  const left = base3({ elev: 20, azim: -50 }).title('Elliptic Paraboloid  z=x²+2y²')
    .add(...axes3(3, GRAY, 0.7), planeZ(f, [-2, 2], [-2, 2]).wire(20, 20).color(B).opacity(0.4),
      point(0, 0, 0).marker('circle').color(R).size(6),
      annotate.text(point(0, 0, 0.5)).label('Vertex (0,0,0)').color(R).font(10));
  const right = base3({ elev: 20, azim: -50 }).title('Cross-Sections: Parabolas  Level Curves: Ellipses').legend('upper left')
  ...
  return pfig([left, right], { cols: 2, title: 'Elliptic Paraboloid — The 3D Bowl', tight: true });
}

// ── 23. Hyperbolic Paraboloid Details (1×2) ──
function hyperbolicParaboloidDetails() {
  const f = (x, y) => x * x - y * y;
  const left = base3({ elev: 20, azim: -50 }).title('Hyperbolic Paraboloid  z=x²−y²')
    .add(...axes3(3, GRAY, 0.7), planeZ(f, [-2, 2], [-2, 2]).wire(20, 20).color(B).opacity(0.4),
  ...
  const right = base3({ elev: 20, azim: -50 }).title('Level Curves: Hyperbolas  Crossing lines at z=0')
  ...
  return pfig([left, right], { cols: 2, title: 'Hyperbolic Paraboloid — The Saddle', tight: true });
}

// ── 24. Four Types of Cylinders (2×2) ──
function cylinderTypes() {
  const circ = base3({ elev: 20, azim: -50 }).title('Circular Cylinder  x²+y²=1')
    .add(...axes3(3.5, GRAY, 0.7), cylS(1, O3, [-3, 3]).wire(30, 15).color(B).opacity(0.4));
  const sinu = base3({ elev: 20, azim: -50 }).title('Sinusoidal Cylinder  z=sin x')
    .add(...axes3(3.5, GRAY, 0.7), planeZ(Math.sin, [-4, 4], [-3, 3]).wire(30, 12).color(B).opacity(0.4));
  const ell = base3({ elev: 20, azim: -50 }).title('Elliptic Cylinder  x²/4+z²/9=1')
    .add(...axes3(3.5, GRAY, 0.7), srf((u, v) => [2 * Math.cos(u), v, 3 * Math.sin(u)], [0, tau], [-3, 3]).wire(30, 15).color(B).opacity(0.4));
  const par = base3({ elev: 20, azim: -50 }).title('Parabolic Cylinder  y=x²')
    .add(...axes3(3.5, GRAY, 0.7), srf((u, v) => [u, u * u, v], [-3, 3], [-3, 3]).wire(24, 12).color(B).opacity(0.4));
  return pfig([circ, sinu, ell, par], { cols: 2, title: 'Four Types of Cylinders', tight: true });
}

// ── 25. Hyperboloid of One Sheet (1×2) ──
function hyperboloidOneSheet() {
  const left = base3({ elev: 20, azim: -50 }).title('Hyperboloid of One Sheet  x²+y²−z²/4=1  Connected')
    .add(...axes3(4, GRAY, 0.7), hyper1S(1, 1, 2).wire(30, 20).color(B).opacity(0.4));
  const right = base3({ elev: 20, azim: -50 }).title('Cross-sections: z=0 (waist), z=1, z=2  All ellipses')
    .add(...axes3(4, GRAY, 0.7), hyper1S(1, 1, 2).wire(30, 20).color(B).opacity(0.3),
      ...[0, 1, 2].map((zv) => circ3(Math.sqrt(1 + (zv / 2) ** 2), zv).color(R).stroke(2)));
  return pfig([left, right], { cols: 2, title: 'Hyperboloid of One Sheet (Connected)', tight: true });
}
...
function hyperboloidTwoSheets() {
  ...
  const left = base3({ elev: 20, azim: -50 }).title('Hyperboloid of Two Sheets  −x²−y²+z²/4=1  Disconnected')
    .add(...axes3(5, GRAY, 0.7), ...sheets());
  const right = base3({ elev: 20, azim: -50 }).title('Gap: |z|<2 has NO real points')
    .add(...axes3(5, GRAY, 0.7), ...sheets(),
      annotate.text(point(0, 0, 0)).label('GAP\n|z|<2\nno points').anchor('middle').color(R).bold().font(12));
  return pfig([left, right], { cols: 2, title: 'Hyperboloid of Two Sheets (Disconnected)', tight: true });
}

// ── 27. Double Cone Details (1×2) ──
function coneDetails() {
  const left = base3({ elev: 20, azim: -50 }).title('Double Cone  z²=x²+y²')
    .add(...axes3(3, GRAY, 0.7), coneS(1, O3, [-2, 2]).wire(30, 20).color(B).opacity(0.4),
      point(0, 0, 0).marker('circle').color(R).size(6),
      annotate.text(point(0, 0, 0.5)).label('Vertex (0,0,0)').anchor('middle').color(R).font(10));
  const right = base3({ elev: 20, azim: -50 }).title('Cross-sections: Circles  Radius r=|z|')
    .add(...axes3(3, GRAY, 0.7), coneS(1, O3, [-2, 2]).wire(30, 20).color(B).opacity(0.25),
      ...[-2, -1, 0, 1, 2].map((zv) => (zv === 0 ? point(0, 0, 0).marker('circle').color('#000').size(6) : circ3(Math.abs(zv), zv).color(R).stroke(2))));
  return pfig([left, right], { cols: 2, title: 'Double Cone — Two Nappes', tight: true });
}
...
function quadricComparison() {
  ...
  return pfig(specs.map(([t, list]) => base3({ elev: 20, azim: -50 }).title(t).add(...list.map((o) => o.color(B).opacity(0.5)))), { cols: 3, title: 'Quadric Surfaces — Complete Gallery', tight: true });
}

// ── 29. Degenerate Quadric Surfaces (1×3) ──
function degenerateCases() {
  const a = base3({ elev: 20, azim: -50 }).title('x²+y²+z²=0 → Single Point (0,0,0)')
    .add(...axes3(2, GRAY, 0.7), frame3([-2, 2], [-2, 2], [-2, 2]), point(0, 0, 0).marker('circle').color(R).size(11));
  const b = base3({ elev: 20, azim: -50 }).title('x²+y²=0 → The z-axis (line)')
    .add(...axes3(2, GRAY, 0.7), frame3([-2, 2], [-2, 2], [-2, 2]), curve3.through([[0, 0, -3], [0, 0, 3]]).color(B).stroke(3));
  const lines = [];
  for (let k = 0; k < 8; k++) {
    const z = -2 + (4 * k) / 7;
  ...
  const c = base3({ elev: 20, azim: -50 }).title('x²−y²=0 → Two planes y=±x')
    .add(...axes3(2, GRAY, 0.7), frame3([-2, 2], [-2, 2], [-2, 2]), ...lines);
  return pfig([a, b, c], { cols: 3, title: 'Degenerate Quadric Surfaces', tight: true });
}
...
function spherePlaneIntersection() {
  ...
  return base3({ elev: 20, azim: -55 }).title('Sphere x²+y²+z²=20 ∩ Plane x+y+z=6  Circle center (2,2,2), r=2√2').add(
    frame3([-1, 5], [-1, 5], [-1, 5]),
    ...axes3(4, GRAY, 0.7),
...
function cylindersIntersection() {
  return base3({ elev: 25, azim: -50 }).title('x²+y²=1 ∩ x²+z²=1  Intersection: Two Crossing Ellipses').add(
...
function lineSurfaceIntersection() {
  return base3({ elev: 20, azim: -55 }).title('Line–Sphere Intersection  (t, t, 5−t) ∩ x²+y²+z²=25').add(
    frame3([-3, 6], [-3, 6], [-2, 7]),
    ...axes3(5, GRAY, 0.7),
...
function symmetry3d() {
  const a = base3({ elev: 20, azim: -50 }).title('z=x²+y²  xz, yz symmetry + rotational about z')
    .add(...axes3(3, GRAY, 0.6), planeZ((x, y) => x * x + y * y, [-2, 2], [-2, 2]).wire(20, 20).color(B).opacity(0.5));
  const b = base3({ elev: 20, azim: -50 }).title('x²+y²+z²=1  All plane symmetries + origin')
    .add(...axes3(1.5, GRAY, 0.6), ballS(1).wire(20, 15).color(B).opacity(0.5));
  const c = base3({ elev: 20, azim: -50 }).title('z=xy  Origin symmetry only (x,y)→(−x,−y)')
    .add(...axes3(3, GRAY, 0.6), planeZ((x, y) => x * y, [-2, 2], [-2, 2]).wire(20, 20).color(B).opacity(0.5));
  return pfig([a, b, c], { cols: 3, title: 'Symmetries of Quadric Surfaces', tight: true });
}
...
function stepIntersection() {
  ...
  return pfig([0, 1, 2].map((i) => {
    const shapes = [frame3([-3, 5], [-3, 5], [-3, 5]), ...axes3(4, GRAY, 0.6), ballS(R20).wire(30, 18).color(B).opacity(0.3)];
    ...
    return base3({ elev: 20, azim: -55 }).title(titles[i]).add(...shapes);
...
function stepQuadrics() {
  ...
  const extraOf = (col) => {
    ...
    if (col === 1) return [1, 2, 3].map((cz) => circ3(Math.sqrt(cz), cz).color(R).stroke(1.5));
    return [0, 1.5].map((zv) => circ3(Math.sqrt(1 + (zv / 1.5) ** 2), zv).color(R).stroke(1.5));
  };
  const list = [];
  for (let row = 0; row < 3; row++) for (let col = 0; col < 3; col++) {
    const shapes = [...(row > 0 ? axes3(3, GRAY, 0.6) : []), ...surfaceOf(col, Math.min(row, 1))];
    if (row === 2) shapes.push(...extraOf(col));
    let sc = base3({ elev: 20, azim: -50 }).add(...shapes);
    const t = row === 0 ? names[col] : (col === 0 ? stages[row] : '');
    if (t) sc = sc.title(t);
    list.push(sc);
  }
  return pfig(list, { cols: 3, title: 'Building Quadric Surfaces — Step by Step', tight: true });
}

// ── run ──
const figs = [
  ['9c-coordinate-system-3d', coordSys3d],
  ['9c-step-3d-coords', step3dCoords],
  ['9c-vector-dot-cross', vectorDotCross],
  ['9c-step-vectors', stepVectors],
  ['9c-plane-intercept', planeIntercept],
  ['9c-plane-normal', planeNormal],
  ['9c-step-plane', stepPlane],
  ['9c-point-plane-distance', pointPlaneDistance],
  ['9c-angle-planes', anglePlanes],
  ['9c-distance-parallel-planes', distanceParallelPlanes],
  ['9c-sphere-details', sphereDetails],
  ['9c-point-sphere-distance', pointSphereDistance],
  ['9c-surface-height-map', surfaceHeightMap],
  ['9c-step-surface-build', stepSurfaceBuild],
  ['9c-domain-regions', domainRegions],
  ['9c-level-curves-method', levelCurvesMethod],
  ['9c-level-curves-to-surface', levelCurvesToSurface],
  ['9c-contour-steepness', contourSteepness],
  ['9c-step-level-curves', stepLevelCurves],
  ['9c-quadric-identification', quadricIdentification],
  ['9c-ellipsoid-details', ellipsoidDetails],
  ['9c-paraboloid-details', paraboloidDetails],
  ['9c-hyperbolic-paraboloid-details', hyperbolicParaboloidDetails],
  ['9c-cylinder-types', cylinderTypes],
  ['9c-hyperboloid-one-sheet', hyperboloidOneSheet],
  ['9c-hyperboloid-two-sheets', hyperboloidTwoSheets],
  ['9c-cone-details', coneDetails],
  ['9c-quadric-comparison', quadricComparison],
  ['9c-degenerate-cases', degenerateCases],
  ['9c-sphere-plane-intersection', spherePlaneIntersection],
  ['9c-cylinders-intersection', cylindersIntersection],
  ['9c-line-surface-intersection', lineSurfaceIntersection],
  ['9c-symmetry-3d', symmetry3d],
  ['9c-step-intersection', stepIntersection],
  ['9c-step-quadrics', stepQuadrics],
];
let ok = 0, fail = 0;
for (const [name, fn] of figs) {
  try {
    ...
    ok++; console.log(`✓ ${name}`);
  } catch (e) { fail++; console.error(`✗ ${name}: ${e.message}`, (e.stack || '').split('\n')[1]); }
}
console.log(`→ ${ok}개 생성, ${fail}개 실패 → output/parity9c/`);
