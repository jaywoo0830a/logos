...
const {
  blue: B, red: R, green: G, magenta: M, orangead: OR,
  gray: GRAY, navy: NAVY, purple: PURPLE, darkgreen: DGREEN,
} = kit.palette;
...
function stepLineForms() {
  ...
  const t = (title, extra = []) => plot2d([-0.5, 7], [-1.5, 5]).title(title).add(
...
function parallelPerp() {
  const p1 = plot2d([-3, 3], [-6, 6]).equal().title('Parallel: m₁ = m₂ = 2').legend()
  ...
  const p2 = plot2d([-3, 3], [-3, 3]).equal().title('Perpendicular: m₁m₂ = -1').legend()
...
function angleBetweenLines() {
  return plot2d([-2, 3], [-1, 6]).title('Angle Between Two Lines')
...
function midpointDivision() {
  const p1 = plot2d([0, 10], [-3, 7]).grid({ alpha: 0.3 }).title('Midpoint')
    .add(seg(point(2, 5), point(8, -1), { color: B }), point(2, 5).marker('circle').color(B).size(5),
  ...
  const p2 = plot2d([-1, 10], [-1, 9]).grid({ alpha: 0.3 }).title('Section Formula & Centroid')
    .add(seg(point(1, 2), point(7, 8), { color: B }), point(1, 2).marker('circle').color(B).size(5),
...
function pointLineDistance() {
  ...
  const t = (title, extra = []) => plot2d([-1, 5], [-0.5, 5.5]).grid({ alpha: 0.3 }).title(title)
...
function stepDistanceLine() {
  ...
  const P = () => point(3, 4).marker('circle').color(R).size(6);
  const t = (title, extra = []) => plot2d([-1, 5], [-0.5, 5.5]).grid({ alpha: 0.3 }).title(title).add(L(), P(), ...extra);
  return subplots([
    t('Step 1: Point & Line'),
...
function twoLinesDistance() {
  return plot2d([-8, 6], [-4, 5]).grid({ alpha: 0.3 }).title('Distance Between Two Parallel Lines').legend('lower right')
    .add(
...
function pointCircleDistance() {
  return plot2d([-3, 8], [-4, 4]).equal().grid({ alpha: 0.3 }).title('Distance from a Point to a Circle')
    .add(
...
function tangentLinesCircle() {
  ...
  return plot2d([-3, 7], [-5, 5]).equal().grid({ alpha: 0.3 })
...
function circleDetails() {
  const p1 = plot2d([-3, 9], [-8, 4]).equal().grid({ alpha: 0.3 }).title('x²+y²−6x+4y−3=0')
    .add(
...
function stepConicCircle() {
  ...
  const t = (title, extra = []) => plot2d([-3, 9], [-8, 4]).equal().grid({ alpha: 0.3 }).title(title)
...
function ellipseDetails() {
  ...
  const p1 = plot2d([-6, 6], [-4, 4]).equal().grid({ alpha: 0.3 }).title('x²/25 + y²/9 = 1')
    .add(E(),
  ...
  const px = 2, py = 2.75;
  const p2 = plot2d([-6, 6], [-5, 4]).equal().grid({ alpha: 0.3 }).title('PF₁ + PF₂ = 2a (constant)')
    .add(E(),
      point(c, 0).marker('star').color(M).size(8), point(-c, 0).marker('star').color(M).size(8),
...
function stepConicEllipse() {
  ...
  const t = (title, extra = []) => plot2d([-7, 7], [-5, 5]).equal().grid({ alpha: 0.3 }).title(title).add(...pts, ...extra);
  return subplots([
...
function parabolaDetails() {
  ...
  const p1 = plot2d([-0.5, 5], [-0.5, 5]).grid({ alpha: 0.3 }).title('y = ½(x−2)² + 1')
  ...
  const p2 = plot2d([-3, 4], [-2, 4.5]).grid({ alpha: 0.3 }).title('PF = distance to directrix')
...
function stepConicParabola() {
  ...
  const t = (title, extra = []) => plot2d([-0.5, 5], [-0.5, 5]).grid({ alpha: 0.3 }).title(title).add(v(), d(), ...extra);
  return subplots([
...
function hyperbolaDetails() {
  ...
  const p1 = plot2d([-8, 8], [-6, 6]).equal().grid({ alpha: 0.3 }).title('x²/9 − y²/4 = 1')
    .add(H(), ...asym(),
      point(a, 0).marker('circle').color(R).size(5), point(-a, 0).marker('circle').color(R).size(5),
      point(c, 0).marker('star').color(M).size(8), point(-c, 0).marker('star').color(M).size(8),
  ...
  const p2 = plot2d([-8, 8], [-6, 6]).equal().grid({ alpha: 0.3 }).title('|PF₁ − PF₂| = 2a')
    .add(H(),
      point(c, 0).marker('star').color(M).size(8), point(-c, 0).marker('star').color(M).size(8),
      point(px, py).marker('circle').color(R).size(6),
      seg(point(px, py), point(c, 0), { color: R, stroke: 1 }).dash([4, 3]),
...
function stepConicHyperbola() {
  ...
  const foci = () => [point(c, 0).marker('star').color(M).size(8), point(-c, 0).marker('star').color(M).size(8)];
  const t = (title, extra = []) => plot2d([-7, 7], [-5, 5]).equal().grid({ alpha: 0.3 }).title(title).add(...asym(), ...extra);
  return subplots([
...
function conicComparison() {
  const circ = plot2d([-5, 5], [-5, 5]).equal().grid({ alpha: 0.3 }).title('Circle')
    .add(circle.center(point(0, 0)).radius(3).color(B).stroke(2.5));
  const ell = plot2d([-6, 6], [-4, 4]).equal().grid({ alpha: 0.3 }).title('Ellipse')
  ...
  const par = plot2d([-4, 4], [-2, 5]).equal().grid({ alpha: 0.3 }).title('Parabola')
  ...
  const hyp = plot2d([-7, 7], [-5, 5]).equal().grid({ alpha: 0.3 }).title('Hyperbola')
...
function parametricMotion() {
  const segp = plot2d([0, 8], [1, 6]).equal().grid({ alpha: 0.3 }).title('Line Segment  (1+5t, 2+3t)')
  ...
  const circ = (() => {
    ...
    return plot2d([-4, 4], [-4, 4]).equal().grid({ alpha: 0.3 }).title('Circle  (3cos t, 3sin t)')
  ...
  const ell = (() => {
    ...
    return plot2d([-5, 5], [-3, 3]).equal().grid({ alpha: 0.3 }).title('Ellipse  (4cos t, 2sin t)')
  ...
  const cyc = plot2d([0, 13], [-0.5, 3]).equal().grid({ alpha: 0.3 }).title('Cycloid  (t−sin t, 1−cos t)')
...
function stepParametric() {
  const circlePanel = (n, title) => {
    ...
    return plot2d([-4, 4], [-4, 4]).equal().grid({ alpha: 0.3 }).title(title)
  ...
  const cycPanel = (n, title) => {
    ...
    return plot2d([0, 7], [-0.5, 2.5]).equal().grid({ alpha: 0.3 }).title(title)
...
function triangleArea() {
  ...
  return plot2d([-1, 6], [-1, 5]).equal().grid({ alpha: 0.3 }).title('Triangle Area — Shoelace Formula')
...
function areaPolygon() {
  ...
  const poly = (s) => s.add(polygonOf(P).fill(B).opacity(0.15))
    .add(polygonOf(P).color(B).stroke(2));
  const p1 = poly(plot2d([-1, 7], [-1, 6]).equal().grid({ alpha: 0.3 }).title('Quadrilateral Vertices'))
  ...
  const p2 = poly(plot2d([-1, 7], [-1, 6]).equal().grid({ alpha: 0.3 }).title('Shoelace: Diagonal Products'))
    .add(
      seg(point(0, 0), point(4, 3), { color: R, stroke: 1 }).dash([5, 4]), seg(point(5, 0), point(1, 4), { color: R, stroke: 1 }).dash([5, 4]),
      seg(point(4, 3), point(0, 0), { color: R, stroke: 1 }).dash([5, 4]), seg(point(1, 4), point(0, 0), { color: R, stroke: 1 }).dash([5, 4]),
...
function pointReflection() {
  return plot2d([-7, 7], [-7, 7]).equal().grid({ alpha: 0.3 })
