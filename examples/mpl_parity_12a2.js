...
function normalPts(seed, n, scale, shift) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    out.push(
      scale.map((s, k) => (k % 2 === 0 ? Math.cos(2 * Math.PI * v) : Math.sin(2 * Math.PI * v)) * m * s + shift[k]),
    );
  }
  return out;
}
...
function matrixTransformation2d() {
  ...
  const left = s2([-0.5, 2.0], [-0.5, 2.0])
    .title('Unit Square (Before)')
    .add(
...
function determinantAreaScaling() {
  ...
  const list = cases.map(([A, head, name]) => {
    ...
    return s2([-3, 5], [-3, 5])
      .title(`${head} — ${name} (|det|=${Math.abs(d).toFixed(0)})`)
      .add(...shapes);
  });
...
function rotationMatrix() {
  ...
  thetas.forEach((th, i) => {
    ...
    shapes.push(
      ...
      annotate
        .angle({ from: v0, vertex: [0, 0], to: vR })
        .arc({ radius: vec.norm(v0) })
        .color(c)
        .stroke(1.5)
        .dash([4, 3]),
    );
  ...
  return s2([-3, 3], [-3, 3])
    .title('Rotation Matrix R(θ): Preserves Length, Changes Direction')
    .legend('lower left')
    .add(...shapes)
    .compile();
}
...
function vectorAddition() {
  ...
  return s2([-0.5, 5], [-0.5, 4.5])
    .title('Vector Addition: Tip-to-Tail (Parallelogram Law)')
    .add(
...
function vectorMagnitude() {
  ...
  return s2([-0.5, 4.5], [-0.5, 5])
    .title('Vector Magnitude: Pythagorean Theorem in Components')
    .add(
      ...
      segment(P([v[0] - 0.2, 0]), P([v[0], 0]))
        .color(INK)
        .stroke(1),
      segment(P([v[0] - 0.2, 0]), P([v[0] - 0.2, 0.2]))
        .color(INK)
        .stroke(1),
...
function vectorProjection() {
  ...
  return s2([-0.5, 5], [-0.5, 3.5])
    .title('Vector Projection: Shadow of a onto b')
    .add(
      ...
      segment(P(b), P(vec.add(proj, vec.scale(vec.unit(b), 0.45))))
        .color(BLUE)
        .stroke(1)
        .dash([3, 3])
        .opacity(0.5),
...
function crossProduct3d() {
  ...
  return plot3d({ elev: 25, azim: -55 })
    .title('Cross Product: a×b ⊥ a and ⊥ b (Right-Hand Rule)')
    .add(
      // 평행사변형 면 ← Poly3DCollection([verts], alpha=0.3)
      surfaceParam((u, v) => [u, v, 0])
        .on([0, 1], [0, 1])
        .solid(1, 1)
        .color(GRAY_L)
        .opacity(0.3),
...
function crossProductArea() {
  ...
  const base = vec.norm(b),
    h = area / base; // mpl: h = |a| sin θ (= |a×b|/|b|)
  const verts = [[0, 0], a, vec.add(a, b), b];
  return s2([-0.5, 5], [-0.5, 4.5])
    .title('|a×b| = Parallelogram Area (in 2D, as scalar in z-direction)')
    .add(
      polyOf(verts, { fill: BLUE, color: BLUE_D, stroke: 2, opacity: 0.25 }),
      arrowAt([0, 0], a, { color: RED, stroke: 2.5 }),
      arrowAt([0, 0], b, { color: BLUE, stroke: 2.5 }),
      segment(P(a), P(vec.add(a, b)))
        .color(BLUE)
        .stroke(1.5)
        .dash([5, 4])
        .opacity(0.5),
      segment(P(b), P(vec.add(a, b)))
        .color(RED)
        .stroke(1.5)
        .dash([5, 4])
        .opacity(0.5),
...
function determinantVolume3d() {
  ...
  return plot3d({ elev: 20, azim: -50 })
    .title(`3D Determinant = Volume Scale Factor\nUnit Cube → Parallelepiped, det = ${A.det.toFixed(2)}`)
    .add(
      // 단위정육면체(점선) ← ax.plot(..., '--')
...
function linearSystemGeometric() {
  const list = [
    ...
    s2([-1, 4], [-1, 4])
      ...
      .add(
        line
          .slopeIntercept(-2 / 3, 5 / 3)
          .color(BLUE)
          .stroke(2)
          .label('2x+3y=5'),
        line
          .slopeIntercept(-1 / 4, 6 / 4)
          .color(RED)
          .stroke(2)
          .label('x+4y=6'),
...
function matrixPowersTransform() {
  ...
  const list = [0, 1, 2, 3, 4].map((k) => {
    const Ak = A.pow(k);
    const quad = Ak.map(UNIT),
      vk = Ak.apply(v0);
    return s2([-3, 3], [-3, 3])
      .title(k === 0 ? 'A⁰ = I (Start)' : `A${k} (Rot ${k}×90°=${k * 90}°)`)
      .add(
...
function projection2dTo1d() {
  ...
  const right = plot2d([-6, 6], [-1, 1], {
    ...
    .grid({ alpha: 0.3 })
    .title('1D Projection (onto x-axis)\nℝ² → ℝ¹ via 1×2 matrix [1 0]')
    .add(
...
function projection3dTo2d() {
  ...
  const left = plot3d({ elev: 20, azim: -60 })
    .title('3D Point Cloud (ℝ³)')
    .add(
...
function dimensionalityCascade() {
  ...
  const cell3d = plot3d({ elev: 20, azim: -50 })
    .title('ℝ⁴ → ℝ³\n(Drop 4th dim)')
    .add(
  ...
  const cell3to2 = s2([-5, 3], [-4, 3])
    .title('ℝ³ → ℝ²\n(Drop z)')
    .xlabel('x')
    .ylabel('y')
    .add(...pts3.map((p) => P([p[0], p[1]]).marker('circle').color(BLUE).size(6)));
  // (1,0) ℝ² → ℝ¹ : x 축으로 정사영
...
function basisTransformationGrid() {
  ...
  const warpedLines = (i0, i1, alpha, width) => {
    ...
    for (let i = i0; i <= i1; i++) {
      out.push(
        segment(P(A.apply([-5, i])), P(A.apply([5, i])))
          .color(GRAY)
          .stroke(width)
          .opacity(alpha),
        segment(P(A.apply([i, -5])), P(A.apply([i, 5])))
          .color(GRAY)
          .stroke(width)
          .opacity(alpha),
      );
  ...
  const left = s2([-3, 5], [-3, 5], { grid: false })
    .title('Original Space (ℝ²)\nRegular grid + unit square')
    .add(
  ...
  const right = s2([-3, 8], [-3, 8], { grid: false })
    .title('Transformed Space (A applied)\nGrid deforms, unit square → parallelogram')
    .add(
